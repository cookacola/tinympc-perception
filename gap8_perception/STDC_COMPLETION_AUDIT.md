...
| Multi-head shared features | Implemented and deployable | shared encoder runs once, then feeds separate corner and danger graphs |
...
| About 100k–180k parameters | Passed | selected shared deployment: 102,933 |
| Roughly 20M–30M MACs | Passed | selected shared deployment: 18,679,680 |
...
| 512 kB L2 firmware fit | Passed | linked shared NanoCockpit image: 225,676 bytes including 180,000-byte workspace; verified peak 153,984 bytes |
...
| 750,000 varied simulated images/tracks | Generator ready; dataset not complete | procedural per-shard course/obstacle/lighting generator passed RGB/depth/semantic smoke tests; launch needs about 64 GiB but only about 20 GiB is free |
| Varying real tracks | Not met | three flights from the available course; measurable between-flight shift |
| GVSOC exact checksum | Pending/too slow | Shared encoder/corner/danger jobs 2848–2850 each remained CPU-bound for 30 minutes after successful link/image preparation, with no terminal checksum; no pass is claimed |
| Crazyflie/TinyMPC integration | Implemented, host-tested | NanoCockpit build passes; TinyMPC equivalence tests 14/14 |
| Full STM32 cross-build | Passed | Bitcraze container build: flash 353,492/1,032,192; RAM 116,212/131,072; CCM 62,248/65,536 bytes |
...
The software, real-gate training, quantization, DORY tiling, linked L2
allocation, decoder, NanoCockpit image, and STM32 integration are reproducible.
The design is not yet physically accepted for 2 m/s racing. That claim requires
the full varied-track corpus, real obstacle labels, completed
simulator/hardware parity (the exact GVSOC runs did not finish in the audit
