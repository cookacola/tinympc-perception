...
def run_epoch(model, loader, device, representation, optimizer=None):
    model.train(optimizer is not None); totals = {k: 0.0 for k in ("loss","bce","dice","sdf_mae","intersection","union","predicted","truth")}; samples = 0
    with (torch.enable_grad() if optimizer is not None else torch.no_grad()):
        for batch in loader:
            ...
            if representation == "binary":
                bce = F.binary_cross_entropy_with_logits(logits, target); dice = soft_dice_loss(logits, target); loss = bce + 0.5*dice
                sdf_mae = logits.new_zeros(())
            else:
                sdf_target = batch["gate_sdf"].to(device, non_blocking=True)
                signed_distance = logits.tanh(); loss = F.smooth_l1_loss(signed_distance, sdf_target)
                sdf_mae = F.l1_loss(signed_distance, sdf_target); bce = logits.new_zeros(()); dice = logits.new_zeros(())
            if optimizer is not None: loss.backward(); optimizer.step()
            prediction, truth, size = logits.sigmoid() >= 0.5, target >= 0.5, image.shape[0]
            totals["loss"] += float(loss.detach())*size; totals["bce"] += float(bce.detach())*size; totals["dice"] += float(dice.detach())*size; totals["sdf_mae"] += float(sdf_mae.detach())*size
    ...
    return {"loss": totals["loss"]/max(1,samples), "bce": totals["bce"]/max(1,samples), "soft_dice_loss": totals["dice"]/max(1,samples), "sdf_mae": totals["sdf_mae"]/max(1,samples),
...
def main():
    ...
    p.add_argument("--representation",choices=("binary","sdf"),required=True)
    ...
    train=MultiTaskDataset(a.dataset,a.targets,a.split_file,"train"); val=MultiTaskDataset(a.dataset,a.targets,a.split_file,"validation")
    ...
    fields=["epoch","seconds","train_loss","val_loss","val_bce","val_soft_dice_loss","val_sdf_mae","val_iou","val_f1","val_precision","val_recall"]; best=-1.0
    with (a.output/"log.csv").open("w",newline="") as stream:
        writer=csv.DictWriter(stream,fieldnames=fields); writer.writeheader()
        for epoch in range(1,a.epochs+1):
            started=time.time(); tr=run_epoch(model,loaders["train"],device,a.representation,optimizer); va=run_epoch(model,loaders["validation"],device,a.representation)
            ...
            print(json.dumps({"tap":a.tap,"representation":a.representation,"epoch":epoch,"train":tr,"validation":va}),flush=True)
            state={"epoch":epoch,"tap":a.tap,"representation":a.representation,"model":model.state_dict(),"optimizer":optimizer.state_dict(),"validation":va,"input":[1,160,160],"output":[1,40,40]}; torch.save(state,a.output/"last.pt")
            if va["iou"]>best: best=va["iou"]; torch.save(state,a.output/"best.pt")
    (a.output/"summary.json").write_text(json.dumps({"tap":a.tap,"representation":a.representation,"best_validation_iou":best},indent=2)+"\n")
